...
const nameMe = userProfile()

export async function getProfile() {
  const profileURL = `${API_SOCIAL_URL}${action}${nameMe}?_listings=true`
...
export async function getUserListings() {
  const ListingsURL = `${API_SOCIAL_URL}${action}${nameMe}${listings}?_seller=true&_bids=true&sort=created&sortOrder=desc`
...
export async function putNewAvatar(newAvatar) {
  const newAvatarURL = `${API_SOCIAL_URL}${action}${nameMe}${media}`
...
export async function getProfileBiddingHistory() {
  const biddingHistoryURL = `${API_SOCIAL_URL}${action}${nameMe}${bid}?_listings=true`
